...
function Box({ position, rotation, args, spring }) {
  ...
  return (
    <a.mesh ref={mesh} position={position} rotation-x={rotX} rotation-y={rotY} rotation-z={rotZ} receiveShadow castShadow>
...
function FloatingBoxes({ inverseColorRef, ...props }) {
  const group = useRef();
  const updateOnProgress = useCallback(
    function updateOnProgress(progress) {
